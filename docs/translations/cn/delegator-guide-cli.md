...
`fetchcli`: 与`fetchd`全节点交互的命令行用户界面。 

::: 安全提示

**请检查并且确认你下载的`fetchcli`是可获得的最新稳定版本**
:::
...
`fetchcli` 需要通过操作系统的终端窗口使用，打开步骤如下所示： 
...
```bash
fetchcli keys add <yourKeyName> --recover
```
...
前，您需要先安装`fetchcli`，同时，您需要知道你希望在哪里保存和使用您的私钥。 最好的办法是把他们保存在一台没有上网的电脑或者一个硬件钱包设备里面。 将私钥保存在一台联网的电脑里面比较危险，任何人通过网络攻击都有可能获取您的私钥，进而盗取您的资产。 
...
当您初始化钱包设备时，设备会产生一个24个词的助记词组。这个助记词组和Cosmos是兼容的，我们可以通过这个助记词组创建Cosmos账户。所以，您需要做的是确认您的钱包设备兼容`fetchcli`，通过下面的步骤可以帮助您确认您的设备是否兼容：
...
```bash
fetchcli keys add <yourAccountName> --ledger 
```
...
```bash
fetchcli keys add <yourKeyName> --recover --account 1
```
...
**在开始设置 `fetchcli`前，请确认你已经可以[访问Cosmos Hub网络](#访问cosmos-hub网络)**
:::

::: 警告

**请确认您使用的`fetchcli`是最新的稳定版本**
:::

无论您是否在自己运行一个节点，`fetchcli` 都可以帮您实现与Cosmos Hub网络的交互。让我们来完成对它的配置。 


您需要用下面的命令行完成对`fetchcli`的配置：
...
// 样例: fetchcli config node https://77.87.106.33:26657
```
...
```bash
fetchcli config chain-id gos-6
```

## 状态查询

::: 提示
** 准备抵押ATOM通证和取回奖励前，需要先完成 [`fetchcli` 配置](#设置-fetchcli)**
:::


`fetchcli` 可以帮助您获得所有区块链的相关信息， 比如账户余额，抵押通证数量，奖励，治理提案以及其他信息。下面是一组用于委托操作的命令
...
fetchcli query staking validator <validatorAddress>


//查询指定地址的验证人相关的所有委托信息 (e.g. cosmos10snjt8dmpr5my0h76xj48ty80uzwhraqalu4eg)
fetchcli query staking delegations <delegatorAddress>
...
fetchcli query staking delegation <delegatorAddress> <validatorAddress>

// 查询一个指定地址的委托人(e.g. cosmos10snjt8dmpr5my0h76xj48ty80uzwhraqalu4eg)所能获得的奖励情况
fetchcli query distribution rewards <delegatorAddress> 

// 查询所有现在正等待抵押的提案
fetchcli query gov proposals --status deposit_period

//查询所有现在正等待投票的填
fetchcli query gov proposals --status voting_period

// 查询一个指定propsalID的提案信息
fetchcli query gov proposal <proposalID>
```
...
::: 提示
**在您抵押通证或取回奖励之前，您需要完成[`fetchcli` 设置](#设置-fetchcli) 和 [创建账户](#创建一个账户)**
:::
...
fetchcli tx staking delegate <validatorAddress> <amountToBond> --from <delegatorKeyName> --gas auto --gas-prices <gasPrice>


// 提取所有的奖励
// 参数设定样例: <gasPrice>=1000uatom

fetchcli tx distribution withdraw-all-rewards --from <delegatorKeyName> --gas auto --gas-prices <gasPrice>
...
fetchcli tx staking unbond <validatorAddress> <amountToUnbond> --from <delegatorKeyName> --gas auto --gas-prices <gasPrice>
```
...
fetchcli tx gov submit-proposal --title "Test Proposal" --description "My awesome proposal" --type <type> --deposit=10000000uatom --gas auto --gas-prices <gasPrice> --from <delegatorKeyName>

// 增加对提案的抵押
// Retrieve proposalID from $fetchcli query gov proposals --status deposit_period
// 通过 $fetchcli query gov proposals --status deposit_period 命令获得 `proposalID` 
// 参数设定样例: <deposit>=1000000uatom

fetchcli tx gov deposit <proposalID> <deposit> --gas auto --gas-prices <gasPrice> --from <delegatorKeyName>

// 对一个提案投票
// Retrieve proposalID from $fetchcli query gov proposals --status voting_period 
通过 $fetchcli query gov proposals --status deposit_period 命令获得 `proposalID` 
// <option>=yes/no/no_with_veto/abstain

fetchcli tx gov vote <proposalID> <option> --gas auto --gas-prices <gasPrice> --from <delegatorKeyName>
```
...
fetchcli tx staking delegate <validatorAddress> <amountToBond> --from <delegatorKeyName> --gas auto --gas-prices <gasPrice> --generate-only > unsignedTX.json
```
...
```bash
fetchcli tx sign unsignedTx.json --from-addr <delegatorAddr>> signedTx.json
```
