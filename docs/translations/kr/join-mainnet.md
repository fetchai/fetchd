...
```bash
fetchd init <your_custom_moniker>
```
...
`moniker`는 `~/.fetchd/config/config.toml` 파일을 통해 추후에 변경이 가능합니다:
...
최소 가스 가격보다 낮은 트랜잭션을 거절하는 스팸 방지 메커니즘을 활성화 하시려면 `~/.fetchd/config/fetchd.toml` 파일을 변경하시면 됩니다:
...
테스트넷의 `genesis.json`파일을 `fetchd`의 config 디렉토리로 가져옵니다.

```bash
mkdir -p $HOME/.fetchd/config
curl https://raw.githubusercontent.com/cosmos/launch/master/genesis.json > $HOME/.fetchd/config/genesis.json
```
...
이제 노드가 다른 피어들을 찾는 방법을 알아야합니다. `$HOME/.fetchd/config/config.toml`에 안정적인 시드 노드들을 추가할 차례입니다. [`launch`](https://github.com/cosmos/launch) repo에 몇 개 시드 노드 링크가 포함되어있습니다.
...
풀노드는 컨펌되지 않은 트랜잭션을 멤풀에 보관합니다. 스팸 트랜잭션으로부터 풀노드를 보호하기 위해서 노드 멤풀에 보관되기 위한 트랜잭션의 최소 가스 가격(`minimum-gas-prices`)을 설정할 것을 권장합니다. 해당 파라미터는 `~/.fetchd/config/fetchd.toml`에서 설정하실 수 있씁니다.
...
```bash
fetchd export > [filename].json
```

특정 블록 높이의 상태를 내보낼 수 있습니다(해당 블록 처리 후 상태):

```bash
fetchd export --height [height] > [filename].json
```

만약 해당 상태를 기반으로 새로운 네트워크를 시작하시려 한다면, `--for-zero-height` 플래그를 이용하셔서 내보내기를 실행해주세요:

```bash
fetchd export --height [height] --for-zero-height > [filename].json
```
...
```bash
fetchd start --assert-invariants-blockly
```
...
invariant broken:
    ...
    CRITICAL please submit the following transaction:
        fetchcli tx crisis invariant-broken staking supply
