...
func newIndex(builder Indexable, prefix byte, indexer *Indexer, indexerF IndexerFunc, indexKey interface{}) (MultiKeyIndex, error) {
	...
	if storeKey == nil {
		return MultiKeyIndex{}, errors.Wrap(ErrArgument, "StoreKey must not be nil")
	}
	rowGetter := builder.RowGetter()
	if rowGetter == nil {
		return MultiKeyIndex{}, errors.Wrap(ErrArgument, "RowGetter must not be nil")
	}
	if indexKey == nil {
		return MultiKeyIndex{}, errors.Wrap(ErrArgument, "RowGetter must not be nil")
	}
	...
	default:
		return MultiKeyIndex{}, errors.Wrap(ErrArgument, "indexKey must be []byte, string or uint64")
	}
