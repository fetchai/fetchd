...
func NewAutoUInt64TableBuilder(prefixData byte, prefixSeq byte, storeKey sdk.StoreKey, model codec.ProtoMarshaler, cdc codec.Marshaler) (*AutoUInt64TableBuilder, error) {
	if prefixData == prefixSeq {
		return nil, errors.Wrap(ErrUniqueConstraint, "prefixData and prefixSeq must be unique")
	}
