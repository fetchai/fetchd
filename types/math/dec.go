...
func NewDecFromString(s string) (Dec, error) {
	...
	if err != nil {
		return Dec{}, errors.Wrap(ErrInvalidDecString, err.Error())
	}
	return Dec{*d}, nil
}
...
func NewNonNegativeDecFromString(s string) (Dec, error) {
	...
	if d.IsNegative() {
		return Dec{}, errors.Wrapf(ErrInvalidDecString, "expected a non-negative decimal, got %s", s)
	}
	return d, nil
}
...
func NewPositiveDecFromString(s string) (Dec, error) {
	...
	if !d.IsPositive() {
		return Dec{}, errors.Wrapf(ErrInvalidDecString, "expected a positive decimal, got %s", s)
	}
	return d, nil
}
