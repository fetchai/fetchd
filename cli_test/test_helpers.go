...
func NewFixtures(t *testing.T) *Fixtures {
	...
	return &Fixtures{
		...
		BuildDir:      buildDir,
		RootDir:       tmpDir,
		GaiadBinary:   filepath.Join(buildDir, "fetchd"),
		GaiacliBinary: filepath.Join(buildDir, "fetchcli"),
		GaiadHome:     filepath.Join(tmpDir, ".fetchd"),
		GaiacliHome:   filepath.Join(tmpDir, ".fetchcli"),
		RPCAddr:       servAddr,
		P2PAddr:       p2pAddr,
		Port:          port,
	}
...
// UnsafeResetAll is fetchd unsafe-reset-all
...
// AddGenesisAccount is fetchd add-genesis-account
...
// CollectGenTxs is fetchd collect-gentxs
func (f *Fixtures) CollectGenTxs(flags ...string) {
	cmd := fmt.Sprintf("%s collect-gentxs --home=%s", f.GaiadBinary, f.GaiadHome)
	executeWriteCheckErr(f.T, addFlags(cmd, flags))
}

// GDStart runs fetchd start with the appropriate flags and returns a process
...
// GDTendermint returns the results of fetchd tendermint [query]
...
// ValidateGenesis runs fetchd validate-genesis
...
// KeysDelete is fetchcli keys delete
...
// KeysAddRecover prepares fetchcli keys add --recover
...
// KeysAddRecoverHDPath prepares fetchcli keys add --recover --account --index
...
// KeysShow is fetchcli keys show
...
//___________________________________________________________________________________
// fetchcli tx send/sign/broadcast

// TxSend is fetchcli tx send
...
// TxStakingCreateValidator is fetchcli tx staking create-validator
func (f *Fixtures) TxStakingCreateValidator(from, consPubKey string, amount sdk.Coin, flags ...string) (bool, string, string) {
	...
	cmd += fmt.Sprintf(" --min-self-delegation=%v", types.DefaultMinSelfDelegation)
	return executeWriteRetStdStreams(f.T, addFlags(cmd, flags), clientkeys.DefaultKeyPass)
}

// TxStakingUnbond is fetchcli tx staking unbond
...
// TxGovSubmitProposal is fetchcli tx gov submit-proposal
...
// TxGovDeposit is fetchcli tx gov deposit
...
// TxGovVote is fetchcli tx gov vote
...
// QueryAccount is fetchcli query account
...
// QueryTxs is fetchcli query txs
...
//___________________________________________________________________________________
// fetchcli query staking

// QueryStakingValidator is fetchcli query staking validator
...
// QueryStakingUnbondingDelegationsFrom is fetchcli query staking unbonding-delegations-from
...
// QueryStakingDelegationsTo is fetchcli query staking delegations-to
...
// QueryStakingPool is fetchcli query staking pool
...
// QueryStakingParameters is fetchcli query staking parameters
...
// QueryGovParamDeposit is fetchcli query gov param deposit
...
// QueryGovParamVoting is fetchcli query gov param voting
...
// QueryGovParamTallying is fetchcli query gov param tallying
...
// QueryGovProposals is fetchcli query gov proposals
...
// QueryGovProposal is fetchcli query gov proposal
...
// QueryGovVote is fetchcli query gov vote
...
// QueryGovVotes is fetchcli query gov votes
...
// QueryGovDeposit is fetchcli query gov deposit
...
// QueryGovDeposits is fetchcli query gov deposits
...
// QuerySlashingParams is fetchcli query slashing params
