...
This integration suite [uses a thin wrapper](https://godoc.org/github.com/cosmos/cosmos-sdk/tests) over the [`os/exec`](https://golang.org/pkg/os/exec/) package. This allows the integration test to run against built binaries (both `fetchd` and `fetchcli` are used) while being written in golang. This allows tests to take advantage of the various golang code we have for operations like marshal/unmarshal, crypto, etc...

> NOTE: The tests will use whatever `fetchd` or `fetchcli` binaries are available in your `$PATH`. You can check which binary will be run by the suite by running `which fetchd` or `which fetchcli`. If you have your `$GOPATH` properly setup they should be in `$GOPATH/bin/gaia*`. This will ensure that your test uses the latest binary you have built
...
- Creates `.fetchd` and `.fetchcli` folders in a new temp folder.
- Uses `fetchcli` to create 2 accounts for use in testing: `foo` and `bar`
...
- Starts `fetchd` and stops it once the test exits
...
- The [`test_helpers.go`](./test_helpers.go) file is organized according to the format of `fetchcli` and `fetchd` commands. There are comments with section headers describing the different areas. Helper functions to call CLI functionality are generally named after the command (e.g. `fetchcli query staking validator` would be `QueryStakingValidator`). Try to keep functions grouped by their position in the command tree.
...
- Sometimes if you exit a test early there can be still running `fetchd` and `fetchcli` processes that will interrupt subsequent runs. Still running `fetchcli` processes will block access to the keybase while still running `fetchd` processes will block ports and prevent new tests from spinning up. You can ensure new tests spin up clean by running `pkill -9 fetchd && pkill -9 fetchcli` before each test run.
