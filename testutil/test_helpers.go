...
func setup(withGenesis bool, invCheckPeriod uint) (*app.App, app.GenesisState) {
	...
	a := app.New(log.NewNopLogger(), db, nil, true, map[int64]bool{}, app.DefaultNodeHome, invCheckPeriod, encCdc, EmptyAppOptions{})
	if withGenesis {
...
// NewSimappWithCustomOptions initializes a new App with custom options.
func NewSimappWithCustomOptions(t *testing.T, isCheckTx bool, options SetupOptions) *app.App {
	t.Helper()
	...
	a := app.New(options.Logger, options.DB, nil, true, options.SkipUpgradeHeights, options.HomePath, options.InvCheckPeriod, options.EncConfig, options.AppOpts)
	genesisState := app.NewDefaultGenesisState(a.AppCodec())
...
func GenesisStateWithSingleValidator(t *testing.T, a *app.App) app.GenesisState {
	t.Helper()
	...
	genesisState := app.NewDefaultGenesisState(a.AppCodec())
	genesisState = genesisStateWithValSet(t, a, genesisState, valSet, []authtypes.GenesisAccount{acc}, balances...)
